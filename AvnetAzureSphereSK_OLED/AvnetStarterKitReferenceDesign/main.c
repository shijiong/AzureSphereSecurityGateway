...
static void UARTEventHandler()
{
	...
	if (bytesRead > 0) {
		...
		json_object_set_number(root_object, "DeviceID", id);
	...
	// Read UART1 message
	bytesRead = read(uart1Fd, receiveBuffer, receiveBufferSize);
	if (bytesRead < 0) {
		Log_Debug("ERROR: Could not read UART: %s (%d).\n", strerror(errno), errno);
		terminationRequired = true;
		return;
	...
	if (bytesRead > 0) {
		// Null terminate the buffer to make it a valid string, and print it
		receiveBuffer[bytesRead] = 0;
		Log_Debug("UART1 received %d bytes: '%s'\n", bytesRead, (char*)receiveBuffer);

		strncpy(deviceID, &receiveBuffer[1], 1);
		id1 = deviceID[0] - 48;

		strncpy(tempValue, &receiveBuffer[2], 2);
		temp1 = (tempValue[0] - 48) * 10 + (tempValue[1] - 48);

		strncpy(humiValue, &receiveBuffer[4], 2);
		humi1 = (humiValue[0] - 48) * 10 + (humiValue[1] - 48);

		strncpy(lightValue, &receiveBuffer[6], 2);
		light1 = (lightValue[0] - 48) * 10 + (lightValue[1] - 48);

		strncpy(gasValue, &receiveBuffer[8], 2);
		gas1 = (gasValue[0] - 48) * 10 + (gasValue[1] - 48);
		...
		json_object_set_number(root_object, "DeviceID", id1);
		json_object_set_number(root_object, "Temperature", temp1);
		json_object_set_number(root_object, "Humidity", humi1);
		json_object_set_number(root_object, "Light", light1);
		json_object_set_number(root_object, "Gas", gas1);
		json_object_set_number(root_object, "PIR", pir1);
...
static int InitPeripheralsAndHandlers(void)
{
	...
	uart0Fd = UART_Open(MT3620_RDB_HEADER2_ISU0_UART, &uartConfig);
	if (uart0Fd < 0) {
		Log_Debug("ERROR: Could not open UART: %s (%d).\n", strerror(errno), errno);
		return -1;
	}
	uart1Fd = UART_Open(MT3620_RDB_HEADER4_ISU1_UART, &uartConfig);
	if (uart1Fd < 0) {
		Log_Debug("ERROR: Could not open UART: %s (%d).\n", strerror(errno), errno);
		return -1;
	}
...
static void ClosePeripheralsAndHandlers(void)
{
	...
	CloseFdAndPrintError(uart1Fd, "Uart");
