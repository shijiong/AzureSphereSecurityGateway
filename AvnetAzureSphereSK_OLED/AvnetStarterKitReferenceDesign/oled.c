...
// Uart sensor values
extern char deviceID[1];
extern char tempValue[2];
extern char humiValue[2];
extern char lightValue[2];
extern char gasValue[2];
extern char pirValue[1];
extern int id;
extern int temp;
extern int humi;
extern int light;
...
void update_oled()
{
	switch (oled_state)
	{
		...
		case 0:
		{
			update_uart_data1(id, temp, humi, light);
		}
		break;
		case 1:
		{
			update_uart_data2(id, gas, pir);
		}
		break;
		//update uart1 data
		case 2:
		{
			update_uart_data1(id1, temp1, humi1, light1);
		}
		break;
		case 3:
		{
			update_uart_data2(id1, gas1, pir1);
		}
		break;
		case 4:
		{
...
void update_uart_data1(int id, int temp, int humi, int light)
{
	// Strings for labels
	uint8_t str_id[] = "Device ID:";
	uint8_t str_temp[] = "Temperature:";
	uint8_t str_humi[] = "Humidity:";
	...
	// Draw the title
	sd1306_draw_string(OLED_TITLE_X, OLED_TITLE_Y, "   Uart", FONT_SIZE_TITLE, white_pixel);

	// Convert id value to string
	ftoa(id, string_data, 0);
	// Draw a label at line 1
	sd1306_draw_string(OLED_LINE_1_X, OLED_LINE_1_Y, str_id, FONT_SIZE_LINE, white_pixel);
	// Draw the value of id
	sd1306_draw_string(sizeof(str_id) * 6, OLED_LINE_1_Y, string_data, FONT_SIZE_LINE, white_pixel);

	// Convert temp value to string
	ftoa(temp, string_data, 0);
	// Draw a label at line 2
	sd1306_draw_string(OLED_LINE_2_X, OLED_LINE_2_Y, str_temp, FONT_SIZE_LINE, white_pixel);
	// Draw the value of temperature
	sd1306_draw_string(sizeof(str_temp) * 6, OLED_LINE_2_Y, string_data, FONT_SIZE_LINE, white_pixel);

	// Convert humidity value to string
	ftoa(humi, string_data, 0);
	// Draw a label at line 3
	sd1306_draw_string(OLED_LINE_3_X, OLED_LINE_3_Y, str_humi, FONT_SIZE_LINE, white_pixel);
	// Draw the value of humidity
	sd1306_draw_string(sizeof(str_humi) * 6, OLED_LINE_3_Y, string_data, FONT_SIZE_LINE, white_pixel);

	// Convert light value to string
	ftoa(light, string_data, 0);
	// Draw a label at line 4
	sd1306_draw_string(OLED_LINE_4_X, OLED_LINE_4_Y, str_light, FONT_SIZE_LINE, white_pixel);
	// Draw the value of light
	sd1306_draw_string(sizeof(str_light) * 6, OLED_LINE_4_Y, string_data, FONT_SIZE_LINE, white_pixel);

	// Send the buffer to OLED RAM
	sd1306_refresh();
}

void update_uart_data2(int id, int gas, int pir)
{
	// Strings for labels
	uint8_t str_id[] = "Device ID:";
	uint8_t str_gas[] = "Gas:";
	...
	// Convert gas value to string
	ftoa(gas, string_data, 0);
	// Draw a label at line 2
	sd1306_draw_string(OLED_LINE_2_X, OLED_LINE_2_Y, str_gas, FONT_SIZE_LINE, white_pixel);
	// Draw the value of temperature
	sd1306_draw_string(sizeof(str_gas) * 6, OLED_LINE_2_Y, string_data, FONT_SIZE_LINE, white_pixel);

	// Convert humidity value to string
	ftoa(pir, string_data, 0);
	// Draw a label at line 3
	sd1306_draw_string(OLED_LINE_3_X, OLED_LINE_3_Y, str_pir, FONT_SIZE_LINE, white_pixel);
	// Draw the value of humidity
	sd1306_draw_string(sizeof(str_pir) * 6, OLED_LINE_3_Y, string_data, FONT_SIZE_LINE, white_pixel);	
